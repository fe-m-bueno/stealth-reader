  ...
  <div class="p-6 bg-vscode-bg text-vscode-text">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="flex items-center gap-2 mb-6 pb-4 border-b border-vscode-border">
        <Filter size={16} class="text-vscode-comment" />
        <span class="text-sm text-vscode-comment mr-2">Filtrar:</span>
        <button
          class="px-4 py-2 rounded text-sm transition-colors {currentFilter === 'all' ? 'bg-blue-500 text-white' : 'bg-vscode-bg/50 text-vscode-text hover:bg-vscode-bg border border-vscode-border'}"
          onclick={() => setFilter('all')}
        >
          Todas
        </button>
        <button
          class="px-4 py-2 rounded text-sm transition-colors {currentFilter === 'today' ? 'bg-blue-500 text-white' : 'bg-vscode-bg/50 text-vscode-text hover:bg-vscode-bg border border-vscode-border'}"
          onclick={() => setFilter('today')}
        >
          Hoje
        </button>
        <button
          class="px-4 py-2 rounded text-sm transition-colors {currentFilter === 'week' ? 'bg-blue-500 text-white' : 'bg-vscode-bg/50 text-vscode-text hover:bg-vscode-bg border border-vscode-border'}"
      ...
      {#if filteredSessions.length === 0}
        <div class="text-center py-16">
          <Calendar size={64} class="text-vscode-comment mx-auto mb-4 opacity-50" />
          <p class="text-vscode-comment text-lg mb-2">Nenhuma sessão encontrada</p>
          <p class="text-sm text-vscode-comment">Comece a ler para registrar sessões!</p>
        </div>
      {:else}
        <div class="space-y-3">
          {#each filteredSessions as session}
            {@const bookTitle = getBookTitle(session.bookId)}
            <div class="bg-vscode-sidebar/50 border border-vscode-border rounded-lg p-4 hover:border-blue-500/50 transition-all">
              <div class="flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 mb-3">
                    <BookOpen size={18} class="text-blue-400 flex-shrink-0" />
                    <h3 class="font-semibold text-vscode-text truncate">{bookTitle}</h3>
                  </div>
                  
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                    <div>
                      <div class="text-vscode-comment text-xs mb-1">Início</div>
                      <div class="text-vscode-text font-mono text-xs">{formatDateTime(session.startTime)}</div>
                    </div>
                    {#if session.endTime}
                      <div>
                        <div class="text-vscode-comment text-xs mb-1">Fim</div>
                        <div class="text-vscode-text font-mono text-xs">{formatDateTime(session.endTime)}</div>
                      </div>
                    {/if}
                    <div>
                      <div class="text-vscode-comment text-xs mb-1">Duração</div>
                      <div class="text-green-400 font-mono font-semibold">
                        {formatDuration(session.duration)}
                      </div>
                      <div class="text-xs text-vscode-comment mt-0.5">
                        {formatDurationDetailed(session.duration)}
                      </div>
                    </div>
                    <div>
                      <div class="text-vscode-comment text-xs mb-1">Palavras</div>
                  ...
                  {#if session.chaptersRead.length > 0}
                    <div class="mt-3 pt-3 border-t border-vscode-border">
                      <div class="text-vscode-comment text-xs mb-1">Capítulos lidos</div>
                      <div class="text-xs text-vscode-text">
        ...
        <!-- Summary -->
        <div class="mt-6 pt-6 border-t border-vscode-border bg-vscode-sidebar/30 rounded-lg p-4">
          <div class="flex items-center justify-between text-sm">
            <div class="text-vscode-comment">
