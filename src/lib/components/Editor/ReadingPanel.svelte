...
<div class="flex flex-col h-full bg-cursor-bg w-full">
  <div
  ...
  >
    {#if session.isLoading}
      <div class="absolute inset-0 flex items-center justify-center bg-cursor-bg/50 z-10">
        <Loader2 class="animate-spin text-cursor-text" size={32} />
      </div>
    {/if}

    {#if session.currentCode.length === 0 && !session.isLoading}
      <div class="flex items-center justify-center h-full text-cursor-text/30 select-none">
      ...
      {#if hasNextChapter()}
        <div class="flex justify-center py-8">
          <button
            ...
            class="flex items-center gap-2 px-6 py-3 bg-cursor-accent hover:bg-cursor-accent-hover text-white rounded-lg transition-colors duration-200 shadow-lg hover:shadow-xl"
