...
<div class="h-full w-full relative">
  <div class="h-full overflow-y-auto editor-scroll-container">
    <div class="p-6 bg-vscode-bg text-vscode-text">
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <button
          class="text-xs text-vscode-comment hover:text-vscode-text px-3 py-1 rounded border border-vscode-border hover:border-vscode-selection transition-colors"
          onclick={loadStats}
        >
          Atualizar
        </button>
    ...
    {#if stats}
        ...
        >
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
              ...
              <span class="text-sm font-medium text-vscode-comment">Tempo Total</span>
            </div>
            <span onclick={(e) => e.stopPropagation()} role="presentation">
              <Tooltip text="Soma de todo o tempo de leitura registrado desde o início. Calculado somando todas as sessões de leitura completadas.">
                <Info size={14} class="text-vscode-comment opacity-0 group-hover:opacity-100 transition-opacity cursor-help" />
              </Tooltip>
            </span>
          </div>
        ...
        <div class="bg-gradient-to-br from-green-500/10 to-green-600/5 border border-green-500/30 rounded-lg p-5 hover:border-green-500/50 transition-all group">
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
              ...
              <span class="text-sm font-medium text-vscode-comment">Palavras Lidas</span>
            </div>
        ...
        <div class="bg-gradient-to-br from-purple-500/10 to-purple-600/5 border border-purple-500/30 rounded-lg p-5 hover:border-purple-500/50 transition-all group">
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
              ...
              <span class="text-sm font-medium text-vscode-comment">PPM Médio</span>
            </div>
        ...
        <div class="bg-gradient-to-br from-orange-500/10 to-orange-600/5 border border-orange-500/30 rounded-lg p-5 hover:border-orange-500/50 transition-all group">
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
              ...
              <span class="text-sm font-medium text-vscode-comment">Sequência</span>
            </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div 
          class="bg-vscode-sidebar/50 border border-vscode-border rounded-lg p-6 hover:border-blue-500/50 transition-all group cursor-pointer"
          onclick={(e) => {
            e.stopPropagation();
        ...
        >
          ...
          <div class="text-sm text-vscode-comment mb-1">
            {#if todayTime === 0}
          ...
          {#if todayTime > 0}
            <div class="mt-4 pt-4 border-t border-vscode-border">
              <div class="text-xs text-vscode-comment">
                Meta diária sugerida: 30 minutos
              </div>
              <div class="w-full bg-vscode-bg rounded-full h-2 mt-2">
                <div
        ...
        <div 
          class="bg-vscode-sidebar/50 border border-vscode-border rounded-lg p-6 hover:border-green-500/50 transition-all group cursor-pointer"
          onclick={(e) => {
            e.stopPropagation();
      ...
      {#if weeklyProgress.length > 0}
        <div class="bg-vscode-sidebar/50 border border-vscode-border rounded-lg p-6 mb-8 group">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold flex items-center gap-2">
          ...
          <div class="space-y-3">
            {#each weeklyProgress.slice().reverse() as week}
              ...
              <div 
                class="group/item cursor-pointer hover:bg-vscode-bg/30 rounded p-2 -m-2 transition-colors"
                onclick={(e) => {
                  e.stopPropagation();
              ...
              >
                <div class="flex items-center justify-between mb-1">
                  <div class="flex flex-col">
                    <span class="text-sm font-medium text-vscode-text">{week.week}</span>
                    {#if weekMatch}
                      ...
                      {@const weekEnd = new Date(weekStart)}
                      {@const weekEndDate = weekEnd.setDate(weekEnd.getDate() + 6)}
                      <span class="text-xs text-vscode-comment">{formatDate(weekStart.getTime())} - {formatDate(weekEndDate)}</span>
                    {/if}
                  </div>
                  <div class="flex flex-col items-end gap-1">
                    <div class="flex items-center gap-4 text-xs">
                      <span class="text-vscode-comment">{formatDuration(week.time * 60 * 1000)}</span>
                      <span class="text-vscode-comment">{formatNumber(week.words)} palavras</span>
                    </div>
                    <span class="text-xs text-vscode-comment">{formatDurationDetailed(week.time * 60 * 1000)}</span>
                  </div>
                </div>
                <div class="w-full bg-vscode-bg rounded-full h-3 overflow-hidden">
                  <div
      ...
      {#if bookMetrics}
        <div class="bg-vscode-sidebar/50 border border-vscode-border rounded-lg p-6 group">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-semibold flex items-center gap-2">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <!-- Tempo Estimado -->
            <div class="bg-vscode-bg/50 border border-vscode-border rounded-lg p-4 hover:border-blue-500/50 transition-all group/item">
              <div class="flex items-center justify-between mb-2">
                <div class="text-sm font-medium text-vscode-comment">Tempo Estimado</div>
                <Tooltip text="Tempo estimado para terminar o livro baseado no seu PPM médio e palavras restantes. Fórmula: (Palavras Restantes × 60) / Seu PPM Médio">
                  <Info size={12} class="text-vscode-comment opacity-0 group-hover/item:opacity-100 transition-opacity cursor-help" />
                </Tooltip>
              </div>
              <div class="text-xl font-bold text-blue-400">{formatDuration(bookMetrics.estimatedTime)}</div>
              <div class="text-xs text-vscode-comment mt-1">
            ...
            <!-- Dificuldade -->
            <div class="bg-vscode-bg/50 border border-vscode-border rounded-lg p-4 hover:border-yellow-500/50 transition-all group/item">
              <div class="flex items-center justify-between mb-2">
                <div class="text-sm font-medium text-vscode-comment">Dificuldade</div>
            ...
            <!-- Seu PPM -->
            <div class="bg-vscode-bg/50 border border-vscode-border rounded-lg p-4 hover:border-purple-500/50 transition-all group/item">
              <div class="flex items-center justify-between mb-2">
                <div class="text-sm font-medium text-vscode-comment">Seu PPM</div>
            ...
            <!-- Palavras Totais -->
            <div class="bg-vscode-bg/50 border border-vscode-border rounded-lg p-4 hover:border-green-500/50 transition-all group/item">
              <div class="flex items-center justify-between mb-2">
                <div class="text-sm font-medium text-vscode-comment">Palavras Totais</div>
            ...
            <!-- Capítulos -->
            <div class="bg-vscode-bg/50 border border-vscode-border rounded-lg p-4 hover:border-cyan-500/50 transition-all group/item">
              <div class="flex items-center justify-between mb-2">
                <div class="text-sm font-medium text-vscode-comment">Capítulos</div>
            ...
            <div class="bg-vscode-bg/50 border border-vscode-border rounded-lg p-4 hover:border-green-500/50 transition-all group/item">
              <div class="flex items-center justify-between mb-2">
                <div class="text-sm font-medium text-vscode-comment">Progresso</div>
          ...
          {#if bookMetrics.lastRead}
            <div class="mt-4 pt-4 border-t border-vscode-border">
              <div class="text-xs text-vscode-comment mb-1">Última Leitura</div>
      ...
      {:else}
        <div class="bg-vscode-sidebar/50 border border-vscode-border rounded-lg p-6 text-center">
          <BookOpen size={48} class="text-vscode-comment mx-auto mb-4 opacity-50" />
          <p class="text-vscode-comment">Nenhum livro aberto no momento</p>
          <p class="text-sm text-vscode-comment mt-2">Abra um livro para ver métricas detalhadas</p>
        </div>
      {/if}
    {:else}
      <div class="text-center py-16">
        <BarChart3 size={64} class="text-vscode-comment mx-auto mb-4 opacity-50" />
        <p class="text-vscode-comment text-lg mb-2">Nenhuma estatística disponível</p>
        <p class="text-sm text-vscode-comment">Comece a ler para gerar estatísticas!</p>
      </div>
    {/if}
      </div>
    </div>
